...
- **Schattenseiten Hauptfläche (v2.0.57 / v2.0.312 / v2.0.371 / v2.0.413–418):** Gegenlicht dimmt die große Wandfront. **v2.0.413–414:** `edgeShade` / `wallUnlit` für Kanten und Rahmen. **v2.0.415–416:** Env/`radiance` + Kamera-Fassade. **v2.0.417–418:** Glas See-through Opacity/Farbe; Unlit aus `sunFromTargetDirection` (nicht `dirLight.getWorldDirection` — Runtime `(0,0,−1)`). Dateien: `facadeShade.ts`, `threeColors.ts`, `elevation.ts`, `main.ts`.
...
- **Werfschatten auf der Wand blass (v2.0.410 / v2.0.411):** Empfang/Cast ok; Density 0,7 → `shadow.intensity` 0,865 (Formel 0,55…1) ließ ~14 % Key in der Umbra. **410:** Mapping **0,90…1**. **Nachweis unzureichend allein:** Rest-Key 0,04, Bounce 0,21 + Env 0,58 blieben. **411:** Density dämpft Bounce und Env-Fill; intensity **0,95…1**. **Nicht:** receiveShadow/PCSS/Contrast-Defaults.
