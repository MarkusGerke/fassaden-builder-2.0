...
**Markisen-Live (v2.0.427):** Während Ein-/Ausfahrt aktualisiert `flushAwningLiveShadowBake` die Sonnen-Map gedrosselt (~48 ms) mit **allen** Castern; Map-Größe temporär max. **4096²**, nach `settleAwningLiveShadow` wieder Präsentation (oft 8192). Details und verworfene Ansätze: [awnings.md](awnings.md).

**Hellerer Look (v2.0.428):** `shadow.intensity` wieder **0,55…1**; Bounce/Env ohne Density-Dämpfung; Fassaden-Gegenlicht wieder `dimMask` (Prä-413); wallUnlit-Env/Glas-Dimmung aus. Markisen-Live unverändert.
...
- **Öffnungs-Zug Commit (v2.0.173 / v2.0.320):** Nach `endOpeningDrag` weiter `live` (kein Material-Invalidate → kein Grau an Profilen). **v2.0.320:** Shadow-Map sofort mit `force: true` — nicht Debounce und nicht hinter Orbit-Lite-Hold (~1 s). Während aktiver Kamerageste bleibt Bake unterdrückt.
...
- **Schattenseiten Hauptfläche (v2.0.57 / v2.0.312 / v2.0.371 / v2.0.428 / v2.0.589 / v2.0.590 / v2.0.593):** Gegenlicht dimmt die große Wandfront (`dimMask`). **v2.0.589:** `dimMask = 1.0` (kein `sideOrTop×0,82`). **v2.0.590:** Tops mit `facadeShadeNormalMode` nutzen bei `upFacing` die Wand-Außenrichtung + `facadeDim = max(..., upFacing·wallBacklit)` — sonst bleibt Gesims-/Sockel-Oberseite hell (Sonne von oben); Cache `facade-backlit-v24`. **v2.0.593:** Dach-Untersicht (`shell`/`tiles`) im Shade-Walk; `downFacing` immer dimmen (Kastentraufe/Plattenunterseite nicht hell neben abgedunkelter Wand); Cache `facade-backlit-v25`. **v2.0.413–418** edgeShade/wallUnlit/Glas-Env — **v2.0.428 zurück** (zu grau). Dateien: `facadeShade.ts`, `threeColors.ts`, `main.ts`.
- **Nacht (v2.0.140 / v2.0.184 / v2.0.185 / v2.0.186 / v2.0.589):** Bei `elevationRad ≤ 0.02` setzt `facadeShadeParamsFromSun` Exterior-Night-Dims (`EXTERIOR_SHADE_NIGHT`, v2.0.589 dunkler: direct 0,05 / hemi 0,035). **v2.0.184:** Sternennacht ohne Ambient-Mittelgrau; Mondlicht bläulich mit Key-Schatten. **v2.0.185:** Außen-EnvMap (`forceExteriorEnv` / Glas) skaliert mit der Tageszeit; **v2.0.589** Sternennacht `envFill` **0,006**. Reflexions-Himmel im Cube-Bake folgt Nacht/Mond. **v2.0.186:** Laibungen nutzen dieselbe Außen-/Innen-EnvMap wie Paneele (kein selektives Schwarz an der Kante).
...
- Licht fällt **nur** durch Fenster- und Tür**öffnungen** und **Glas** in den Innenraum. Wände, Decken, Böden und Fensterrahmen bleiben lichtdicht (siehe [Lichtdichte Hülle](#lichtdichte-hülle-sonne)). **v2.0.349–v2.0.351:** Innen-Dims + kein Innen-EnvMap — wirkte visuell nicht, weil der Patch vor `lights_fragment_end` lief (Hemisphere ungedimmt). **v2.0.352:** `applyInteriorShadeShader` nach `lights_fragment_end`; `dirLightIndoor` ohne 0,28-Floor. **v2.0.361:** Sonnen-Key nicht doppelt mit `intensity/2.4` skalieren (steckt schon in `celestial.lightIntensity`) — Fassade wirkte flach/weiß. **v2.0.360:** Nach Reload/Tageszyklus hohe Sonnenhöhe mit `intensity` ~0,04 → kein Key-Schatten; `repairStaleSunIntensity` + Latch nur noch Mond an `lightIntensity`. **v2.0.353:** Innen-Patch nur Indirect — Direct ungedimmt (Laibung zu hell). **v2.0.359:** Shader v6 — Indirect enger, Direct moderat (`interiorDirectDim` ~0,24–0,44), Sonnenfleck bleibt. **v2.0.354:** Innen-**Boden**-Platten zusätzlich Außen-Layer (Sonnenflecken); Decken nur Innen. **v2.0.350:** Hemisphere/Bounce nur Layer Exterior; `INTERIOR_SHADE_NIGHT`.
...
- **Manuelle Overrides:** `#sun-azimuth` (Himmelsrichtung), `#sun-elevation` (Höhe), Intensität, Weichheit, Farbtemperatur — bleiben, bis Datum/Tageszeit wieder den Solar-Look schreibt (beide Winkel).
...
- **Sonnenlicht** Default **3,0** (Slider `#sun-intensity` 0,3…**8**, v2.0.369). Zusätzlich **Umgebungslicht** (`#sun-ambient`, Default **0,05**), **Schatten-Kontrast** (`#sun-shadow-contrast`, Default **0,50**, Bereich **0,5…10**), **Schatten-Tiefe Fassade & Innen** (`#sun-shade-depth`, Default **≈0,92** / altes 0,75, v2.0.419 — UI 0…1 = Wirk −2…1, links ≈3× mehr Aufhellung), **Schatten-Weichheit** (`#sun-softness`, Default **2,0**), **Farbtemperatur** (`#sun-color-temp`, Default **3500 K**), Tageszeit **13:15**, Sonnenwinkel **210°** und Schatten-Dunkelheit (`#sun-shadow-density`, Default **0,70**, Legacy versteckt). **v0.7.253:** Diese Slider steuern auch Umbra-Tönung, Kontaktschatten und Bodenreflex — siehe [lighting-mood.md](lighting-mood.md). **v2.0.207 / v2.0.256:** Contrast dimmt Hemi (`facadeShade`) und Boden-Umbra — hohe Werte liefern deutlich dunklere Schatten. **v2.0.258:** Density setzt zusätzlich `dirLight.shadow.intensity` (damals 0,55…1). **v2.0.410/411:** kurz Mapping **0,95…1** + Bounce/Env-Dämpfung — **v2.0.428 zurück** auf 0,55…1 ohne Density-Dämpfung (hellerer Look). **v2.0.364 — Schatten-Tiefe (Fassade & Innen)** (`#sun-shade-depth`, `SunSettings.shadeDepth`, Default **0,50**, 0…1): dunkelt die **sonnenabgewandte Fassadenseite** (Gegenlicht-Shader `hemiDim`/`directDim`) und den **Innenraum** (`interiorDirectDim`/`interiorHemiDim`/`interiorIndirectGain`) — `applyShadeDepth` in `facadeShade.ts`; 0 = Rohkurve, 1 = fast schwarz. Wirkt **nicht** auf Boden-Schlagschatten (`shadow.intensity`, Density/Kontrast). **v2.0.419:** 0 = Aufhellung über Rohkurve (Legacy −2), ≈⅔ = Rohkurve, 1 = schwarz; Alt-Werte gemappt. **v2.0.365:** Fensterrahmen/Sprossen/Bänke nutzen den Gegenlicht-Shader im **Normalen-Modus** (`facadeShadeNormalMode`, N·L statt Objekt-Z) — statt `skipFacadeShade`; sonnenseitig unverändert, Schattenseite folgt dem Slider. **v2.0.366:** Innen dimmt der Slider nur Streulicht (Hemi/Env, `1 − 0,92·d`), Direct nur `1 − 0,3·d` — Sonne durch Öffnungen/Glas bleibt sichtbar (Tag-Basis `interiorDirectDim` 0,6+0,15·a). Glas-EnvMap-Basis 2,6 → **1,0** (`GLASS_ENV_BASE_*`, `threeColors.ts`): vorher überstrahlte die Himmel-Spiegelung den dunklen Innenraum („innen hell“ war Glas). **v2.0.367:** Innen-Streulicht `hemiDim × gain` statt `hemiDim² × gain` (≈0) — Weiß bleibt hellgrau lesbar, nicht dunkelgrau.
- **Sonnen-Slider (v0.7.341 … v2.0.335):** Azimut/Tageszeit/Intensität: Licht und Himmel **sofort** (max. 1×/Frame). **Scrub (v2.0.202 → v2.0.335):** Frustum jedes Frame; Sonnen-Shadow **gedrosselt** (~200 ms, `flushSunScrubShadowBake`) — v2.0.202 backte jedes Frame und blockierte flüssigen Tag/Nacht-Übergang. Final beim Loslassen (`change`) + EnvMap. **Szene abspielen (v2.0.255):** gleicher Scrub-Pfad (`sunLiveScrubActive`). Tagzyklus Debounce ~120–280 ms. Live kein EnvMap-Bake und keine Punktlicht-Cubes beim Scrub. Geometrie-Commit weiter `flushSunShadowMap({ sceneLights: true })`.
- **Fenster in 2D (v0.7.344):** Rahmen/Konsolen empfangen Werfschatten wenn Paneele empfangen (`syncOpeningReceiveShadows`, ohne Glas). **v2.0.9:** Teil-Rebuild muss `syncLabelShadowReceivers` rufen — sonst bleiben neue Rahmen ohne Empfang. **v2.0.12:** Erstes Shadow-Map-Bake nach `loadMeshes` (`bootstrapSceneLighting`) — ohne Reload-Workaround über Sonnenwinkel-Slider. **v2.0.357:** Wechsel zu **3D/Fassade** + erster Bootstrap mit `forceShadowBake` — sonst stale/leere Map nach Orbit-Defer bis Sonnen-Slider bewegt wird.
...
| Paneele / Mörtel | Paneele: ja; **Mörtel: nein (v2.0.231)** — sonst harte Fugen-Schatten auf den Steinen | **2D-Front: ja** (v0.7.285 Werfschatten); **3D Farbe: ja** (v2.0.90); Zeichnung/Streiflicht aus |
...
| `src/lighting/pcssShadows.ts` | PCSS-ShaderChunk (nähegewichtete Blocker-Suche + variable Penumbra, `const`-Poisson-Disk + `mat2`-Rotation, entrollte Taps, **Receiver-Plane-Bias** v2.0.252 — auf Distanz mit `texelUv/footprint` gedämpft v2.0.270, **Distanz-AA** Radius ≥ Pixel-Footprint × `PCSS_FOOTPRINT_SCALE` **1,0** v2.0.314 (zuvor 0,5), **Kontakt-Blend** `mix(soft, hard, contact)` unter ~2 Texeln v2.0.260 — ersetzt `min(hard, soft)` v2.0.258; Early-Out für voll lit / volle Umbra — Umbra nur nah / Penumbra-dominiert v2.0.314; Distanz: feste Rotation + lit statt ungefiltertem Hart-Tap), Lichtgröße aus Weichheit; **kein** Orbit-1-Tap mehr (v2.0.197) |
...
| Softness-Default | 2,0 | v2.0.369 (zuvor 2,5); Slider bis 8 weitet die Penumbra |
...
- **Weichheit am Maximum bei hoher Sonne (v2.0.268):** Persistiertes `shadowSoftness: 8` trotz Elevation ~50° (Solar-Weichheit ~2) → riesiger PCSS-Radius, Rauschen/„Geflackere“ beim Orbit, matte Flächen wirken düster. `normalizeSunSettings` setzt Soft≥7,5 auf elevSoft zurück, wenn elevSoft&lt;4 (tiefe Sonne mit Soft 8 bleibt). Nicht verwechseln mit bewusster Weichheit am Horizont.
- **Weichheit nur Penumbra / helle Überlagerung (v2.0.269 → zurück v2.0.270):** Versuch: Early-Out Umbra ohne `filterRadius <= searchRadius` + `softDrive` (Kontakt-Hartanteil mit `pcssLightSizeUv`). Symptom danach: Raster/Poisson auf Paneelen und Glas. Fix v2.0.270: Early-Out und Kontakt-Blend wieder wie v2.0.260; `softDrive` entfernt. `min(hard, soft)` weiterhin verboten.
- **Raster/graue Flecken beim Rauszoomen (v2.0.270 / v2.0.314 / v2.0.315):** Steine, Glas, Profile, Sockel zeigten auf Distanz punktierte Fugen und halbgraue Steine (Render; Vorschau ohne PCSS sauber → PCSS-Ursache). Receiver-Plane-Bias (v2.0.252) aus Screen-Ableitungen: deckt ein Pixel mehrere Shadow-Texel ab, laufen `dFdx/dFdy` über Stein-/Profilkanten → Steigung Unsinn (bis Clamp ±6) → Taps einer Seite gelten als Blocker → Filter halbgrau. Fix v2.0.270: `pcssReceiverPlaneSlope` dämpft mit `clamp(texelUv / footprint, 0, 1)`; Such-/Filterradius ≥ `footprint × PCSS_FOOTPRINT_SCALE`. **v2.0.314:** (1) Ohne Blocker Distanz→lit statt ungefiltertem Hart-Tap. (2) Umbra-Early-Out nur Penumbra-dominiert. (3) Poisson-Rotation auf Distanz fest. (4) `PCSS_FOOTPRINT_SCALE` **1,0**. (5) Adaptives `camera.near` (3 % Distanz, max 150 cm). **Nicht** Distanz-Empfangs-Gate (kurz in v2.0.314 ab 1000 cm Empfang aus) — killte Schlagschatten auf Paneelen; **v2.0.315** wieder Empfang in 3D an. Nicht: Weichheit/Density/Penumbra-Defaults anfassen, Slope-Bias ganz entfernen, PCSS-Taps reduzieren, `min(hard, soft)` zurück.
...
- **Konche pechschwarz (v2.0.401):** Shadow-Tunnel hatte zusätzlich eine Kappe auf der **Mundöffnung** (`outerZ`). Sonne sah eine geschlossene Wand → Kalotte volle Umbra. **Nicht:** `receiveShadow` aus (dann fehlen Gesims-/Laibungs-Schatten in der Nische). **Fix:** Mundkappe entfernen; nur Rückwand- (+ flach: Innenkanten-)Kappe wie bei eckigen Nischen (`panelGeometry.ts`).
...
- **v2.0.231:** Mörtel-Fugen (`lodTier: mortar`) **werfen keinen Schatten** mehr (`castShadow: false`), empfangen weiter. Harte Fugen-Muster auf Paneelen entfallen; Gesims-/Fenster-Schatten auf Paneelen bleiben weich wie auf der Wand.
...
- **Erker-Schenkel dunkler als Schlagschatten auf der Wand (v2.0.312).** Symptom: Innenecke Erker — Schenkel pechgrau, angrenzende Fassade im Schlagschatten mittelhell; Gesims-Schatten ebenfalls stark unterschiedlich.
  - **Ursache:** Gegenlicht-Shader dämpfte Hemi auf sonnenabgewandten Hauptflächen (~0,39); PCSS-Schlagschatten behalten volles Ambient.
  - **Nicht:** PCSS/Density/Softness-Defaults ändern; nicht nur Erker `skipFacadeShade`.
  - **Fix:** `hemiDim` anheben; etwas mehr Bounce. Direct weiter gedimmt.
...
- **Werfschatten auf der Wand blass (v2.0.410 / v2.0.411) → zurück v2.0.428:** Kurz Mapping **0,95…1** und Bounce/Env-Dämpfung. Nutzer wollte wieder den helleren Look → Intensity **0,55…1**, Bounce/Env voll; Markisen-Live (427) unberührt.
...
- **Licht durch Erker-Boden/Deckel (v2.0.258 / v2.0.263):** `baySoffit` castete nicht (nur Empfang) → Sonne durch Untersicht (**258**). **263:** Tagsüber setzte `applyIndoorShadowCasting` den Soffit nur auf Innen-Layer → Sonne sah ihn nicht → Flecken an der Wand unter dem Erker. Fix: Soffit immer Layer 0+1 + Cast, auch ohne Punktlicht-Okklusion.
- **Helle Flecken unter dem Erker trotz 263 (v2.0.264):** Glas castet nicht → Sonne sieht durch die Erker-Fenster in den Raum; diese Loch-Texel liegen in Map-UV nur ~56 cm über der Fuge, der PCSS-Filter (Erker-Front als Blocker → großer Radius) zählt sie als lit. **Nicht:** Shader-Lochfilter (Look-Änderung, zurückgenommen), 120-cm-Brüstung (Loch nur auf 78 cm verschoben). **Fix:** `bayMouthSunOccluder` — unsichtbare Blende in voller Geschosshöhe in der Erker-Mundöffnung, nur Sonnen-Layer (`FacadeController`, `BAY_MOUTH_SUN_OCCLUDER_INSET_CM`). Details: `bay-windows.md`.
