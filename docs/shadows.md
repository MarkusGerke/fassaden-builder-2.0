...
- **Sonnen-Slider (v0.7.341 / v2.0.148):** Azimut/Tageszeit/Intensität: Licht und Himmel **sofort**; Sonnen-Shadow gedrosselt (Debounce ~90 ms, Max-Lag 250 ms). Live kein EnvMap-Bake und keine Punktlicht-Cubes (die hängen nicht am Sonnenstand). EnvMap + finaler Sonnen-Shadow beim Loslassen (`change`). Geometrie-Commit weiter `flushSunShadowMap({ sceneLights: true })`. **Weichheit (v2.0.1):** `#sun-softness` → PCSS-Lichtgröße 0,8…28 cm als Uniform `pcssLightSizeUv` (live). **Farbtemperatur (v2.0.1):** `#sun-color-temp` färbt das Key-Light.
