...
| `src/lighting/pcssShadows.ts` | PCSS-ShaderChunk (nähegewichtete Blocker-Suche + variable Penumbra, `const`-Poisson-Disk + `mat2`-Rotation, entrollte Taps, **Receiver-Plane-Bias** v2.0.252 — auf Distanz mit `texelUv/footprint` gedämpft v2.0.270, **Distanz-AA** Radius ≥ Pixel-Footprint × `PCSS_FOOTPRINT_SCALE` 0,5 v2.0.270, **Kontakt-Blend** `mix(soft, hard, contact)` unter ~2 Texeln v2.0.260 — ersetzt `min(hard, soft)` v2.0.258; Early-Out für voll lit / volle Umbra), Lichtgröße aus Weichheit; **kein** Orbit-1-Tap mehr (v2.0.197) |
...
- **Raster/graue Flecken beim Rauszoomen (v2.0.270):** Steine, Glas, Profile, Sockel zeigten auf Distanz punktierte Fugen und halbgraue Steine (Render; Vorschau ohne PCSS sauber → PCSS-Ursache). Receiver-Plane-Bias (v2.0.252) aus Screen-Ableitungen: deckt ein Pixel mehrere Shadow-Texel ab, laufen `dFdx/dFdy` über Stein-/Profilkanten → Steigung Unsinn (bis Clamp ±6) → Taps einer Seite gelten als Blocker → Filter halbgrau. Fix: `pcssReceiverPlaneSlope` dämpft mit `clamp(texelUv / footprint, 0, 1)` (nah = 1, unverändert); Such- und Filterradius mindestens `footprint × PCSS_FOOTPRINT_SCALE` (0,5 → Filterdurchmesser ein Pixel, Distanz-AA). Nicht: Weichheit/Density/Penumbra-Defaults anfassen, Slope-Bias ganz entfernen (Schraffur auf Gesims/Sockel kommt zurück), PCSS-Taps reduzieren.
