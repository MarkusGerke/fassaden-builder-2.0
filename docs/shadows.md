...
- **Schattenseiten Hauptfläche (v2.0.57 / v2.0.312 / v2.0.371):** `dimMask = 1 − sideOrTop` — bei Gegenlicht werden große Wandflächen abgedunkelt; Seiten/Oberkanten der Geometrie bleiben hell. **v2.0.312:** `hemiDim` hoch (~0,55…0,94) — Ambient bleibt auf Gegenlicht-Flächen weitgehend erhalten (sonst Erker-Schenkel pechschwarzer als Schlagschatten auf der Nachbarwand). Direct weiter stark gedimmt. **v2.0.370–371:** horizontale Facetten im Schatten: `dimMask` mit Faktor **0,82** plus zusätzliche Indirect-Dimmung über `facadeSideOrTop` (Cache `facade-backlit-v14`). Datei: `facadeShade.ts`.
