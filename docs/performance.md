...
- **Sonne live (v2.0.148–v2.0.150 / v2.0.182 / v2.0.201 / v2.0.202 / v2.0.255):** Tageszeit/Azimut: Himmel+Key sofort. **Manueller Scrub (v2.0.202):** Sonnen-Shadow jedes Lighting-Frame (rAF); Debounce (~120–280 ms) nur noch Tagzyklus — sonst Sprünge. **Szene abspielen (v2.0.255):** gleicher Scrub-Pfad im Hauptloop (`sunLiveScrubActive`), kein Extra-rAF, Toolbar ~10 Hz. Scrub: Gebäudebox gecacht, Frustum fitten, Map-Größe/EnvMap/Schedule-Actors erst bei `change`/Stop. Dämmerung/Schedule: Auto-Lichter soft (`enabled` + `fadeFactor`, kein `applyState`); Okkluder bleiben bei existierenden Lichtern; Re-Enable ohne Cube-Rebake.
