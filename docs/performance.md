...
- **Sonne live (v2.0.148–v2.0.150):** Tageszeit/Azimut: Himmel+Key sofort; Live-Schedule nur Sonnen-Shadow (kein EnvMap, keine Punktlicht-Cubes); Debounce mit Max-Lag 250 ms. Dämmerung/Schedule: Auto-Lichter soft (`enabled` + `fadeFactor`, kein `applyState`); Okkluder bleiben bei existierenden Lichtern; Re-Enable ohne Cube-Rebake.
- **Orbit-Lite (v2.0.100 / v2.0.124 / v2.0.125 / v2.0.151 / v2.0.170):** Bei **jeder** Kamerageste: EnvMap-Bake pausiert; Gizmo-Update im Loop pausiert. **Pixelratio:** standardmäßig 1 während der Geste; **Ausnahme:** Bloom an und „Bloom bei Kamerabewegung aus“ nicht gesetzt → volle Pixelratio. **Bloom:** bleibt standardmäßig **an**. **PCSS (v2.0.170):** während Orbit wieder `pcssLite` (1 Tap, Uniform) — Idle weich; EnvMap erst über Shadow-Scheduler nach Gestenende (kein sofortiger 1‑s-Hänger). Hold `ORBIT_LITE_HOLD_MS` (320 ms).
...
- **PCSS-Lite (v2.0.120, Idle v2.0.151, Orbit wieder v2.0.170):** Uniform `pcssLite` — während Orbit 1, Idle 0.
