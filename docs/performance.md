...
- **Orbit-Lite (v2.0.100):** Bei **jeder** Kamerageste: Pixelratio 1 (Renderer **und** EffectComposer); **Bloom aus** während der Geste (Composer-Pfad spart Frames); EnvMap-Bake pausiert; Gizmo-Update im Loop pausiert (der `change`-Listener aktualisiert die Greifer weiterhin — billig, `getBoundingClientRect` nur mit Auswahl). PCSS bleibt an (kein Shader-Umschalten mittendrin; adaptiv 1 Tap, siehe v2.0.120). Mausrad: Three.js `start`+`end` im selben Tick — Lite bleibt `ORBIT_LITE_HOLD_MS` (320 ms) nach dem letzten Event. Nach Ende wieder Bloom (wenn aktiv) und volle Pixelratio.
