...
- **Orbit-Lite (v2.0.100 / v2.0.124 / v2.0.125 / v2.0.151 / v2.0.170 / v2.0.172 / v2.0.196 / v2.0.197 / v2.0.251 / v2.0.259):** Bei **jeder** Kamerageste: EnvMap-Bake pausiert; Gizmo-Update im Loop pausiert. **v2.0.259:** auch Shadow-Map-Bake während Orbit unterdrückt (Timeout/Flush/Label-Dirty → nachholen am Ende) — sonst stockt die Geste an 8192²; Maß-Label-Skalierung und Laub-Tick pausiert; `setPixelRatio` nur bei Cap-Wechsel. **Pixelratio:** Entwurf/Vorschau → 1 während der Geste, Idle **1,5**; **Render** und Bloom (ohne „bei Bewegung aus“) → **`MAX_PIXEL_RATIO_RENDER` = 2** auch während Orbit (sonst wirken weiche Schatten hart; v2.0.251 höher gegen Treppchen). **Bloom:** bleibt standardmäßig **an** beim Orbit — Szene direkt auf den Canvas, Bloom additiv. Hold `ORBIT_LITE_HOLD_MS` (**1000 ms**, v2.0.251). **v2.0.197:** kein PCSS-Lite-Pfad mehr im Shader; Cursor-Rule gegen Regression.
