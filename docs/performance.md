...
- **Orbit-Lite (v2.0.100 / v2.0.124 / v2.0.125 / v2.0.151):** Bei **jeder** Kamerageste: EnvMap-Bake pausiert; Gizmo-Update im Loop pausiert. **Pixelratio:** standardmäßig 1 während der Geste; **Ausnahme:** Bloom an und „Bloom bei Kamerabewegung aus“ nicht gesetzt → volle Pixelratio. **Bloom:** bleibt standardmäßig **an**. **PCSS:** weich wie Idle (adaptives 1-Tap-Lite entfernt, v2.0.151). Hold `ORBIT_LITE_HOLD_MS` (320 ms).
...
- **PCSS-Lite (v2.0.120, abgeschaltet v2.0.151):** Uniform `pcssLite` bleibt im Shader (Tests/Debug), wird beim Orbit aber nicht mehr auf 1 gesetzt — Navigation und Idle nutzen volles PCSS.
