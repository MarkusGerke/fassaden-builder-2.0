...
- **Wind nicht als Idle-Killer (v2.0.474 / v2.0.476):** `tickWindFabrics` darf `liveMotion`/`viewportDirty` **nicht** setzen. Tick erst, wenn dieser Frame ohnehin gerendert wird. **v2.0.476:** Licht/EnvMap/Orbit wieder wie v2.0.473 (Nutzer-A/B: gleiche JSON dort flüssig). Dach-Toggle: nur Dach-Meshes, kein Fassaden-Rebuild. Nicht PCSS-Lite/DPR-1; nicht EnvMap nach jedem Edit weglassen (Himmel ändert sich).
...
- **Sonne live (v2.0.148–v2.0.335):** Tageszeit/Azimut: Himmel+Key sofort. **Manueller Scrub (v2.0.202 → v2.0.335):** Früher Sonnen-Shadow **jedes** Lighting-Frame (8192²) — Tag/Nacht-Slider stockte. **v2.0.335:** Shadow-Bake beim Scrub max. ~5×/s (`SUN_SCRUB_SHADOW_BAKE_MIN_MS` 200), final beim `change`; Himmel/Licht weiter jedes Frame. **Szene abspielen (v2.0.255):** gleicher Scrub-Pfad (`sunLiveScrubActive`). Scrub: Gebäudebox gecacht, Frustum fitten, Map-Größe/EnvMap/Schedule-Actors erst bei `change`/Stop. Tagzyklus weiter Debounce ~120–280 ms.
- **Blaulicht (v2.0.335):** Blitzdauer skaliert mit geglätteter Frame-Zeit (`blaulichtFlashDurationMs`) — bei niedriger FPS sichtbar statt zwischen Frames verloren.
- **Shader-Lichtanzahl (v2.0.336):** `stableLightCount` wieder für **alle** platzierten Lichter (`sceneLightShaderCountStable`) — Ein/Aus ohne Programm-Rebuild (~5 s UI-Freeze ab v2.0.335). Vorrats-Reserven (`padSpareLights`) weiter nur im Licht-Modus (v2.0.260); im Render nur inaktive Slots, keine +4-Leerlichter.
- **Orbit-Lite (v2.0.100 / … / v2.0.259 / v2.0.261 / v2.0.262 / v2.0.320):** Bei **jeder** Kamerageste: EnvMap-Bake pausiert; Gizmo-Update im Loop pausiert. **v2.0.259:** auch Shadow-Map-Bake während Orbit unterdrückt; Maß-Labels und Laub pausiert; `setPixelRatio` nur bei Cap-/Transmission-Wechsel. **v2.0.320:** Geometrie-Commit (Wand/Öffnung verschieben) setzt `forceShadowBakePending` — ein Bake trotz Hold, sonst bleiben Schatten ~1 s + Debounce alt. **Pixelratio:** Entwurf/Vorschau → 1 während der Geste, Idle **1,5**; **Render idle** → `MAX_PIXEL_RATIO_RENDER` = **2**; **Render Orbit** → `MAX_PIXEL_RATIO_RENDER_ORBIT` = **1,5** (v2.0.261; DPR 1 macht PCSS hart — verboten). **v2.0.262:** auch mit Bloom an (früher erzwang Bloom DPR 2 → Orbit wieder stockig). **Transmission:** während Orbit `transmissionResolutionScale` **0,5** (Idle 1). Hold `ORBIT_LITE_HOLD_MS` (**1000 ms**). **v2.0.197:** kein PCSS-Lite.
...
- **3D-Bögen:** Clip/SVG bleiben bei `ARCH_CURVE_SEGMENTS = 128`; Extrude/Shape, Keilsteine und Paneel-Bogenkappen-Front nutzen `ARCH_MESH_SEGMENTS = 64` (v2.0.273; Voussoir weiterhin 6–8 Segmente je Stein).
...
| `transmissionResolutionScale` 0,5 | ≈ 20 | großer Hebel — **umgesetzt v2.0.261** nur während Orbit (Glas kurz weicher) |
...
**Umsetzung v2.0.261 / v2.0.262:** Orbit-Transmission 0,5 + Render-Orbit-DPR 1,5 — **auch mit Bloom** (v2.0.262; zuvor Vollratio 2 bei Bloom). Erwartung ≈ 16–22 ms/Frame ohne Bloom; mit Bloom etwas höher wegen Composer, aber nicht mehr DPR-2-gebunden.

Fazit: Fragment-gebunden; Kosten ≈ (Lichter + PCSS) × (Haupt-Pass + Transmission-Pass) × Pixel. Weitere Hebel sind Produktentscheidungen: DPR 1,5 idle im Render, einfaches Glas statt physikalischer Transmission.
