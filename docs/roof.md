...
- **Giebel als Wand (v2.0.589–v2.0.593):** Vertikale Giebelfüllung (`gableFill` / `roofPart: shell`) wählt die **Host-Wand** (LMB + RMB-Wandmenü), keine Dach-Toolbar. Waagerechte Untersicht/Deckel bleiben Dach. Mit Paneelen/Mauerwerk auf der Giebelwand deckt das Raster das Dreieck; die Dach-Füllwand auf der Wandlinie entfällt (`skipFillEdgeIndices`). Kastentraufe bleibt. **v2.0.590:** Paneel-Meshes mit `wallPlacement(layoutWall)` — sonst Y-Versatz `(H_ext−H)/2`. **v2.0.591:** Mörtelplatte mit `clipRectBandToGableProfile`. **v2.0.592:** Clip-Y = Dach-**Unterseite** (`envelopeY − tv`), nicht Oberhaut; Trauf-Cladding teilt `roofWallTopTrimCm` mit dem Wandkörper — sonst Kantenlinien durch die Schräge. **v2.0.593:** Shell/Tiles im Gegenlicht-Shade; nach unten zeigende Flächen (`downFacing`) immer dimmen.
