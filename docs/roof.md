...
- **Giebel als Wand (v2.0.589):** Vertikale Giebelfüllung (`gableFill` / `roofPart: shell`) wählt die **Host-Wand** (LMB + RMB-Wandmenü), keine Dach-Toolbar. Waagerechte Untersicht/Deckel bleiben Dach. Mit Paneelen/Mauerwerk auf der Giebelwand deckt das Raster das Dreieck bis unter die Dachhaut; die Dach-Füllwand auf der Wandlinie entfällt (`skipFillEdgeIndices`). Kastentraufe bleibt.
