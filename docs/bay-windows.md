...
- **Untersicht / Soffit (item 6 / v2.0.239):** Unter jedem echten Erker (kein Balkon/Loggia) wird die Unterseite automatisch geschlossen — man sieht nicht mehr von unten durch den Erker. Erzeugt in `FacadeController.rebuildIndoorFloor()`: Außenpolygon aus Schenkel- + Frontwänden (Planlinie = Außenkante), extrudiert am Erker-Fuß (`host.y`). **Zusätzlich Deckel an der Erker-Oberkante**, wenn darüber kein fortgesetzter Erker sitzt. Farbe = **`wallColor`**. Von außen sichtbar (Exterior-Layer), wirft **keinen** Schatten, empfängt Schatten (`userData.kind = 'baySoffit'`). Zusätzlich bekommen Erker-Wände in `createStudioWallGeometry` **immer** den vollen Boden (keine Türlücken), sonst fehlt die Wandstärke-Unterseite.
