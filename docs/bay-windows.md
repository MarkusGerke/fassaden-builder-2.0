...
- **Sockel auf Schenkeln zu hoch / Rock schwarz (v2.0.233 → Fix v2.0.234 / Regression v2.0.442).** Symptom: Nach „Nach unten verlängern“ saß der Sockel auf linker/rechter Erkerseite über dem Restwand-Sockel; der verlängerte Rock wirkte schwarz statt in Wandfarbe.
  - **Ursache:** Rock-Zone blieb auf der paneel-eingesunkenen Außenfläche (wirkt unter Licht schwarz); Skirt-Offset allein aus `dropCm` konnte von der realen Y-Verlängerung abweichen. **v2.0.442:** zusätzlich Gegenlicht-Shader auf der Rock-Schale → pechschwarz trotz voller Tiefe.
  - **Nicht geholfen:** Nur Host-Sockel / nur Meta-`dropCm` ohne Messung; `skipFacadeShade` allein ohne Material-Neubau (Shader blieb).
  - **Fix:** `bayWallSkirtDropCm` (und Profil-Pendant) = Abstand Restwand-Fuß bei gleicher Oberkante; bei Skirt Außenfläche volle Tiefe; Soffit `wallColor`; **v2.0.442:** Exterior-Material bei Skirt neu ohne Shade.
