...
Leibung: `createStudioOpeningRevealGeometry` (Nische mit Rückwand bei `fill.mode === 'niche'`). **Konche** (`type: 'conch'`): `createStudioConchRevealGeometry` — Halbzylinder unter der Kämpferlinie + Viertelkugel-Kalotte; Wandloch über erzwungene Rundbogen-Maske (`openingArchForm` → `round`). **v2.0.284/285:** `panelWrappedReveal` — Return-Steine pro Stein aus dessen Randkette (`WrapMiterSpec` → `wrapChainsOfRing` → `extrudeWrapReturn`, Loft: Körper von der Steinfront `zOf(P)` bis `zSplit` + beidseitig gefaster Boss, Bogen folgt den Steinpunkten); Fassadensteine enden an der Öffnungskante (Boss-Seite flach; der 45°-Versatz in `makeCoordAt` aus v2.0.284 ist in v2.0.285 entfernt — Keile an Bogen ∩ Lagerfuge). Außen-Quads bleiben als Mörtelbett (Gruppe 0), Sohlbank Putz (Gruppe 2). Details: [facade-layers.md](facade-layers.md).
