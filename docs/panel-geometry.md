...
`cornerJoin: 'none'` (Default, UI tot seit v2.0.153): **Gehrung der Verkleidung nur bei Fortsetzung derselben Schicht** um die Ecke. Ohne Paneele/Mauerwerk auf dem Nachbarn → stumpf und bündig an der Plan-Kante (`±width/2`), kein Keil-Überstand; Stirn der Steine bleibt sichtbar. Der **Wandkörper** gehrt weiter über `miterStart`/`miterEnd` (Volumen-Overlap vermeiden). Mit Paneel-Nachbar: Bilderrahmen-Gehrung wie bisher. **v2.0.252.** **v2.0.303/304:** Erker — erst stumpf (Lücke), dann wieder Gehrung + Vorstand 0 für Planbreite = sichtbare Front. **v2.0.307:** Vorstand 0 zurückgenommen — Steine ohne Dicke lagen 0,15 cm vor der Wandschale und Z-fighteten (Streifen ab ~10 m); Erker haben wieder Vorstand 4 / Bosse 1, Raster bleibt per v2.0.306 auf `wall.width`. **v2.0.305:** eigentliche Ursache war Innen-Origin der Erker-Wände (Körper nach außen, +2×Wandstärke); Erker-Umlauf liegt jetzt immer auf der Außenkante (`docs/bay-windows.md`). **v0.7.281:** Forced-Ends nur an **45°-Knicken** (komplementär 0,5/1). Front-Layout (`mapFrontCutsToPlan`) nur bei **Innen-Origin** (`panelFlip: false`, sichtbare Front länger als der Plan) oder an **45°-Knicken**; bei Außen-Origin + 90°-Gehrung bleibt das Raster **immer** auf `wall.width` (**v2.0.306** — die Paneelfront steht dort um `projectDepth` vor der Kante und ist an der Gehrung um 2×projectDepth *länger*, früher kippte das ins Front-Layout mit 8-cm-Stummel). **v0.7.280:** Load-Fit wie 73dbdc9 — Origin auf der Außenecke, Außenlinie in Planrichtung (kein 40-cm-Stummel). **v0.7.279:** Verknüpfte Wände mit Origin innen (`panelFlip: false`) werden beim Laden auf die Außenkante gelegt. **v0.7.278:** Raster auf der sichtbaren Außenfront; Steine dürfen `wallX < 0` (Keil vor der Plan-Kante). Feld und Öffnungen ungehrt. Der Clip an der Laibung bleibt am Öffnungsmaß.
...
8. **v2.0.252:** `panelMiterEnds` / `plinthMiterEnds` / `corniceMiterEnds` / Zierband gehren nur bei **gleichem Dekor** auf dem Nachbarn — sonst stumpf an der Plan-Kante. Sockel-/Gesims-/Zierband-Profile ohne Fortsetzung: **Stirnkappe** (`capStart`/`capEnd`), kein offenes Hohlprofil. Wandkörper-Gehrung unverändert. **Sockelprofil-/Gesims-/Zierband-Sweep** (v0.7.220): `x += z × (−tan)` wie `wallLocalX`, vorzeichenbehafteter Knick. **Zierband-Löcher (v0.7.241):** `openingMaskXRangesAtY` — dieselbe Maske wie Paneele/Mauerwerk (inkl. Rundbogen), plus Aufweitung um Rahmenprofil.

**Läuferverband** (`runningBond`): Gerade Reihen `1/1/1…` mit Restbreite **nur am Ende** (v2.0.68 — vorher symmetrisch auf beide Enden → Drift zwischen 24er/48er); versetzte Reihen `0,5/1/…` ebenfalls Start exakt, Rest am Ende. **v2.0.297 End-Stretch auf Erker rückgängig in v2.0.298** — statt verzerrter 1,125-Endsteine lieber passende Bibliothek-Breiten (**v2.0.302:** 288 schmal / 384 breit). Das Muster gilt **pro Wand**, auch wenn die Steinbreite die Wand- oder Nachbarlänge nicht teilt (v0.7.174). `⅓`/`¼`-Läufer nutzen die echte Endstückbreite, nicht nur ein ½-Raster. An **45°-Ecken** erzwingen `diagonalBondEndWidth` + `buildRunningBondWithForcedEnds` komplementäre Endsteine 0,5/1; das Feld dazwischen nur volle Läufer (kein zweiter Halbstein nach dem Forced-Start). **v0.7.296:** welche Wand den 0,5-Stein in geraden Lagen bekommt, hängt von **Yaw/Origin** ab — nicht von `wall.id`. Sonst kippt der Verband nach Etage duplizieren oder Stil kopieren (neue UUIDs), und die obere Etage wirkt wie ein anderes Raster. **v2.0.31:** Bossen-Einzug und Stirn entfallen an Plan-Kante und Rechteck-Laibung — sonst Reißverschluss in der Zeichnung (Halbstein vs. Vollstein). **v2.0.82:** An der Laibung keine Stummel schmaler als das Verbandmodul (½ Läufer): `absorbJambSlivers` nach `pierceWallCutsToField`.
...
#### Muster-Defaults beim Auswählen (v2.0.259)

Beim Klick auf eine Musterkarte oder Bibliothek-Ablegen setzt `studioPanelDefaultsForPattern` die Maße (bestehende Wände behalten beim reinen Load ihre Werte):

| Muster | Breite × Höhe | Vorstand | Reihen oben | Bossen |
|---|---|---|---|---|
| Streifen | 64 × 32 | 4 | 3 | — |
| Läuferverband (+ ⅓/¼-Varianten) | 48 × 24 | 4 | 0 | Tiefe 1, Profil 0,8 |
| Kopfverband | 24 × 8 | 4 | 0 | Tiefe 0 |

`DEFAULT_STUDIO_PANEL` = Streifen + Sockel **64 × 8**. Datei: `src/studio/constants.ts`.
...
**Bogenkappe / Distanz-Zacken (v2.0.166 → v2.0.273/276):** Clip liefert weiterhin feine `bottomArc`/`topArc` (~128). Die **sichtbare Front** ist ein Earcut-Polygon (`arcCapFrontRing`), keine Spalten-Quads. Stützpunkte: Stride von Clip-`ARCH_CURVE_SEGMENTS` (128) auf `ARCH_MESH_SEGMENTS` (64) — **ohne** Douglas-Peucker (v2.0.274; DP-Toleranz 0,85 ließ auf Paneel-Abschnitten lange Sehnen neben dem feinen Blendrahmen). **v2.0.275:** reiner Index-Stride zerstörte Rechtwinkel an L-/Boss-Ringen (`lostCorners`) → scharfe Knicke (`isSharpPolylineTurn`) bleiben beim Dock-Stride. **v2.0.276:** dichte Boss-Ringe (~100 pts) lieferten `offsetLoops` ohne Deckfläche (`tops:0` → offene Front); Boss-Einzug sparst auf ≤40 mit Ecken, sonst flacher Fallback. Soffit und Bogen-`remnantOutline` ebenso. 128 Spalten-Quads bleiben verboten (Zoom-Zacken).

**Erker-Drop / Schichtflucht (v2.0.276 / v2.0.277 / v2.0.278):** `visiblePanelRowRange` / Layout nutzen `bayWallSkirtDropCm` als Raster-Ursprung — Schichten fluchten mit der Restwand; zuvor nur Clip ab Skirt bei Raster vom verlängerten Fuß. **v2.0.278:** Messung auch bei Meta-`dropCm: 0` (Desync Geometrie↔Meta); Early-Return 0 aus v2.0.277 entfernt.

Öffnungen werden durch `subtractRect` aus den Außen-/Innenflächen herausgeschnitten. Die **Leibung** liegt in `createStudioOpeningRevealGeometry` (pro Öffnung; **v0.7.247:** Außenhälfte `wall.wallColor`, Innenhälfte `wall.interiorColor`, zwei Material-Gruppen in `FacadeController.rebuildReveals`) und spannt von `studioOpeningRevealInnerZ` (= `studioWallInnerLocalZ`, v0.7.193) bis `studioOpeningRevealOuterZ` — je Maskenkante zwei Quads (Außen-/Innenhälfte, v0.7.239 ohne Lippe/Soffit). **v2.0.247 / v2.0.249:** `REVEAL_JAMB_INSET_CM` **0,25** cm — Maske `openingMaskPolyline(opening, -inset)`; Laibung `polygonOffset` Factor +1 / Units **−6** (`REVEAL_DEPTH_UNITS`, v2.0.250: gewinnt distanzunabhängig gegen Stein-Seitenflächen; Units +1 ließ Ziegel bei Distanz durch). (v2.0.248 Inset 0,8 + Factor −1 erzeugte sichtbare rote Kante — zurückgenommen.) **v2.0.243:** Farbtrennung bei Fenster/Tür an der Fensterfront (`studioOpeningRevealColorSplitZ`, +2 cm hinter dem Rahmen) — nicht mehr geometrische Mitte (sonst dunkler Innenfarben-Streifen vor dem Blendrahmen). **v2.0.186:** Außenhälfte `finishExteriorMaterial` (gleiche CubeCamera-EnvMap wie Paneele), Innenhälfte `finishInteriorMaterial`; Nische/Konche weiter beidseitig Interior. Mit Freiraum endet sie an der Vertiefungskante (`studioClearanceRecessZ`) plus nur `REVEAL_CLEARANCE_INSET_CM` (0,12 cm, v2.0.117 — früher 0,6 cm ließ eine Lichtspalte). Freiraum-Kappe empfängt Schatten. **v0.7.292 / v2.0.115 → v2.0.244:** `REVEAL_OUTER_INSET_CM` war 0,6 cm hinter der Paneelfront; seit v2.0.244 **0** (bündig) mit `polygonOffset +1` an der Laibung — der Rücksprung war als Absatz sichtbar. `WINDOW_FRAME_OVERLAP_CM = 0` (Rahmen = Lochmaß; 1,5 cm Überstand half nicht gegen den Absatz). Sichtbare Front = `studioVisibleOutwardLocalZ(wall, { treatAsBareWall })`: bei Fassadenschmuck „Paneele aus“ die Wandkante — dann auch `createStudioWallGeometry` ohne 0,15-cm-Einzug, Außenbank/Bank-Profil und Schatten-Tunnel an derselben Kante.
...
**Streifen an der Öffnung (v2.0.583):** Laibung und Sturz, egal welche Bogenform, sind die Maske, die das Paneel schneidet. `extrudeStripCutByMask` legt die Bossenfront auf diese Kontur. Die Fase läuft nur an waagerechten Schichtfugen (Kante mindestens 6 cm, `|dy| ≤ 0,45` cm) und endet auf der Maske. Rechteck-Streifen haben keinen seitlichen Einzug. In der Zeichnung baut `coalesceStripDrawingLines` jede Schichtkante neu und schneidet sie mit `openingMaskPolyline`; Mesh-Kanten der Fase kommen nicht in die Linie. Unterbrochen wird eine Fuge nur von einer Wandöffnung.
...
Leibung: `createStudioOpeningRevealGeometry` (Nische mit Rückwand bei `fill.mode === 'niche'`). **Konche** (`type: 'conch'`): `createStudioConchRevealGeometry` — Halbzylinder unter der Kämpferlinie + Viertelkugel-Kalotte; Wandloch über erzwungene Rundbogen-Maske (`openingArchForm` → `round`). **v2.0.401:** Shadow-Tunnel ohne Mundkappe an der Fassadenöffnung (nur Rückwand-/Innenkanten-Kappe) — sonst pechschwarze Kalotte. **v2.0.284/285:** `panelWrappedReveal` — Return-Steine pro Stein aus dessen Randkette (`WrapMiterSpec` → `wrapChainsOfRing` → `extrudeWrapReturn`, Loft: Körper von der Steinfront `zOf(P)` bis `zSplit` + beidseitig gefaster Boss, Bogen folgt den Steinpunkten); Fassadensteine enden an der Öffnungskante (Boss-Seite flach; der 45°-Versatz in `makeCoordAt` aus v2.0.284 ist in v2.0.285 entfernt — Keile an Bogen ∩ Lagerfuge). Außen-Quads bleiben als Mörtelbett (Gruppe 0), Sohlbank Putz (Gruppe 2). Details: [facade-layers.md](facade-layers.md).
