...
**Modulraster & Felder (v2.0.35):** `computeRowColCuts` legt jede Lage **feldweise**: `courseFields` teilt die Lage an Laibungskörpern unter der Kämpferlinie (`openingJambSealHoles` → `rowJambBlockers`) in Felder Wandende ↔ Laibung ↔ Laibung ↔ Wandende. Jedes Feld wird mit `moduleCourseCuts` im Modulraster gelegt: Feldlänge = N Einheiten (`patternModuleUnit`: ½ Läufer, ⅓, ¼, ½ Binder), Steine minimal gedehnt/gestaucht, innere Fugen auf `phase + j·k`. Endstücke sind dadurch **immer** ganze Einheiten (1 / 0,5 Läufer), nie 1,5er oder Splitter; der Versatz zwischen den Lagen ist exakt. Verbände mit Läufer- **und** Binderlagen (`patternModuleGranularity` = 2) runden N auf gerade Zahlen, damit die Läuferlage nicht mit ¼ endet. Kacheln, die ganz in einem Laibungskörper liegen, entfallen bereits im Layout; `mergeNarrowClipParts` hat an Laibungen nichts mehr zu tun. Reststück-Regel: Rest wandert ans freie Feldende; neben einem **lagenweise wechselnden** Zwangs-Endstein (45°-Ecke, Bossen `alternate`) wird stattdessen das Feld gedehnt (kein „H H“ neben dem Endstein). Dock-Kette: der Start einer Wand mit kollinearem Vorgänger spiegelt dessen Endstein (`startForce`/`startAlt`, rekursiv bis `MAX_DOCK_CHAIN`); eine Wand **vor** einer Dock-Naht ohne eigenen Start-Zwang wird von der Naht her gelegt (Rest am freien Anfang). Fallstrick: Nachbar-Schnitte immer über `rowColCutsWithOpenings` holen (inkl. dessen Laibungsfelder), sonst stimmt die Naht nicht.
...
**Keile über der Bogenkappe (v2.0.35):** `clipPolyMinusColumnHole` verwirft Spalten nicht mehr einzeln, wenn ihr Rest über dem Bogen dünner als `ARCH_REMNANT_CRUMB_CM` (3,2 cm) ist. Das erzeugte am auslaufenden Keil zwischen Bogen und Schichtoberkante eine flache Stufe — sichtbar als dunkler „Kasten“ über dem Scheitel (Streifenmuster, breite Steine). Jetzt entscheidet die **zusammenhängende Gruppe**: Stücke unter dem Loch (Sohlbank-Zwickel), die nirgends 3,2 cm erreichen, fallen weg; Stücke **über** dem Loch (`ClipBand.aboveHole`) bleiben immer und laufen dem Bogen folgend bis zur Schichtoberkante aus. `emitMonotone` überspringt für solche Keile den AABB-Höhen-Krümelfilter.
