...
**Modulraster & Felder (v2.0.61 / v2.0.62):** `computeRowColCuts` legt das **Verbandmuster wandweit** (klassisch: Läufer `1/1/1…` bzw. versetzt `0,5/1/…/0,5` über `buildStretcherCuts` / `buildOffsetStretcherCuts`; 45°/Dock über `buildRunningBondWithForcedEnds`). `courseFields` liefert nur **Pfeiler** (nicht die Öffnung); `pierceWallCutsToField` schneidet das Raster an den Laibungen — innere Steine behalten die Sollbreite, Fugen fluchten über die ganze Wand. Ecken-/Bogen-Clip und Jamb-Siegel bleiben. **v2.0.62:** kein per-Feld-Dehnen mehr; Clip-Mindestbreite für Merge/Split gedeckelt, damit 64×32-Streifen und -Zwickel nicht zerfallen.
