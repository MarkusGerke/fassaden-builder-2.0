...
**Öffnungen an Fugen (v2.0.66–v2.0.68):** `masonryPatternCuts` + `openingPanelSnap.ts`. Drag snapt absolut zur Maus (`mode: 'drag'`, vom Drag-Start aus); Nudge/`←→` springt zur nächsten Fuge (`mode: 'nudge'`). Gestapelte Etagen mit verschiedenem Modul → LCM-Raster. **v2.0.68:** Läufer-Rest nur am Ende → 24/48-Fugen fluchten. Streifen/Wild/none: 8-cm-Raster.
