...
8. Rechts in `#toolbar-scene-light`: Voreinstellung, Abstrahlung, Winkel, Position, Farbe, Leistung (Watt LED), Marker, Reichweite, Abfall, Schatten.
...
1. **Cube-Shadows:** Wände, Verkleidung, Rahmen, Laibung, Geschossplatten und unsichtbare Außenring-Okkluder (`SHADOW_LAYER_OCCLUDER`) werfen in die Punktlicht-Cube-Map (`customDistanceMaterial` wo nötig). **v2.0.107:** Okkluder 100 cm dick + Stoßplatte 160 cm zwischen Etagen; `normalBias` 0,25 cm; Indoor-Platten und `sunCeilingOccluder` casten bei Raum-Okklusion immer. **v2.0.111 / v2.0.118:** 2D-Front Cube-Map **4096**; Soft-Würfel-Filter entfernt — Hard-Cube; `shadow.camera.far` folgt Reichweite/`distance` (unbegrenzt großzügig), damit keine imaginäre Box das Licht abschneidet.
...
| `src/lighting/sceneLightRuntime.ts` | Point- und SpotLight(s) + Marker/Pick |
