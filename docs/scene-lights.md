...
7. **Ebenenbaum (links):** Sektion **Lichter** — Einträge heißen nach Art (`Laterne`, `Blaulicht 2`, …). **Shift+Klick** = Bereichsauswahl aller sichtbaren Zeilen zwischen Anker und Klick (auch Wände/Öffnungen, v2.0.183); **Ctrl/Cmd+Klick** = einzelne Mehrfachauswahl → Mehr-Menü **Gruppieren**. Gruppenzeilen aufklappbar; Klick wählt alle Mitglieder. Pro Licht (⋯ oder Rechtsklick): **Ein-/Ausblenden**, **Duplizieren**, Aus Gruppe lösen, **Entfernen** — bei Mehrfachauswahl für alle gewählten. Sektions-Mehr-Menü: **Punktlicht einfügen**, **Alle ein-/ausblenden**, **Alle löschen**. Manuelles Alle ausblenden bleibt an (Auto-Sonne/Schedule greifen erst wieder nach „Lichter mit Sonne“ oder Verlassen des Licht-Modus). **v2.0.178 / v2.0.192:** Klick auf ein Licht (oder jedes andere Objekt) in der Szene markiert die Zeile im Ebenenbaum (Sektion wird aufgeklappt).
...
| **Bloom bei Kamerabewegung aus** | aus (unchecked) | Wenn an: Bloom während Orbit/Zoom aus. Default: Bloom bleibt auch beim Navigieren an. **v2.0.125:** Bei Bloom an und dieser Option aus blieb die Pixelratio beim Orbit voll. **v2.0.262:** Orbit nutzt wieder DPR 1,5 auch mit Bloom (sonst stockig). **v2.0.172:** grauer Kasten war Himmel-Depth im Composer, nicht Bloom-Aus. |
