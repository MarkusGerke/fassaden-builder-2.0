...
Im Akkordeon **Szene** (rechte Leiste) bei **Neutral** (Viewport-Wechsler Himmel | Neutral): **Hintergrund** und **Bodenfarbe**-Picker (v2.0.209; **v2.0.325:** sichtbarer Außenboden immer Steingrau, Picker ohne Effekt auf den Mesh). **Pfützen** (`#ground-puddles-enabled` + `#ground-puddles-options`): Anzahl/Größe/Abstand/Spiegelung (v2.0.326), siehe [ground-puddles.md](ground-puddles.md). Ausgeblendet (IDs bleiben): Umgebung-Duplikat, Laub, „Alle drei“, Himmelsfarbe. Persistenz unter `PersistedAppState.scene` / `puddles`. **v2.0.554:** Sektion `data-settings-section="scene"` (nicht `colors`) — sonst globaler Farben-Hide. Details: [stage-environment.md](stage-environment.md).
...
**Schicht-Editor (v2.0.495):** Unter Fassade → Maße Checkbox **„Schicht setzen“**. Bibliothek bewaffnet den Stein; Klick neben der Wand dreht 0°/90°; orangene Reihe → Domino → `courseOverrides`. Farbstufe 0–7 = Index in derselben Kontrast-Palette. Details: [masonry-course-editor.md](masonry-course-editor.md).
...
- **Farbe (v0.7.247 / v2.0.243):** Tunnel in Außen- und Innenhälfte — **Außenhälfte** = `wall.wallColor` (bzw. `revealExteriorColor`), **Innenhälfte** = `wall.interiorColor`. Bei Fenster/Tür liegt die Trennung an der **Fensterfront** (+ 2 cm nach innen, `studioOpeningRevealColorSplitZ`), nicht am geometrischen Mittel — sonst war die Innenfarbe vor dem Blendrahmen als dunkler Streifen sichtbar. Leere Durchbrüche weiter 50/50. **v2.0.406:** versiegelte Nische/Konche durchgängig Wandfarbe. Zwei Material-Gruppen in `createStudioOpeningRevealGeometry`. Pick als Öffnung (`openingPart: 'group'`).
...
- **Bündig mit der Öffnungskante (v2.0.244–249):** `REVEAL_OUTER_INSET_CM = 0`, Laibung `polygonOffset` **+1**. **v2.0.247/249:** `REVEAL_JAMB_INSET_CM = 0,25` (0,8 in v2.0.248 erzeugte rote Stein-Kante — zurück). **v2.0.248/249:** Profile Bias 0,2 / Clearance 0,5 / Offset −16 gegen Distanz-Z-Fight. Sichtbare Front über `studioVisibleOutwardLocalZ`; Decor „Paneele“ → Geometrie-Rebuild.
...
- Gesims, Zierband, Sockelprofil, Rahmen, Außenbank und Verdachung sitzen auf `studioProfileAnchorLocalZ` → Paneelfläche (`projectDepth`, ohne Bossen-Trapez). Negatives `offsetForward` zieht nicht hinter die Fläche. Ohne Paneele / Decor „Paneele“ aus / Freistreifen: Wandaußenkante (`useWallOuterFace` / `treatAsBareWall`).
...
- **v2.0.550:** Schließen des Übernehmen-Toasts synchronisiert mit `hideScopePropagateOffer` (kein hängendes Pending).
- **v2.0.550:** Toast schließen (×) leert Pending über `onStatusChange` → `#scope-propagate-dismiss` (nicht `toaster.subscribe` — API fehlt in Zag).
- **v2.0.549:** Sichtbar ist der Ark-Toast (`ScopeOfferToast` / `Toaster`), nicht die geclippte Vanilla-Leiste. Logik und Buttons bleiben `#scope-propagate-*`. Einklappen links/rechts und Nav-Hilfe sitzen nicht mehr in der oberen Chrome-Zeile.
...
**Desktop Park-Shell (v2.0.539 / v2.0.540 / v2.0.569 / v2.0.570 / v2.0.571 / v2.0.572):** untere Bibliothek **nur bei Auswahl sichtbar** (ohne Auswahl `data-open=false`, Overlay zu). **v2.0.571/572:** Dock bleibt immer gemountet (auch Stage/`ui-bottom-collapsed` via `data-dock-collapsed`), sonst gehen die Kachel-Hosts verloren. Slide-in als Overlay (`translateY` only, 280 ms) — die Bühne ändert die Höhe nicht, sonst blitzt der Canvas. Kompass (`#view-compass`) rückt bei Auswahl um die Dock-Höhe nach oben. Ohne Auswahl volle Bühne. **v2.0.569:** rechte Spalte mutual exclusive — mit Auswahl nur Objekt-Inspector, ohne Auswahl Szene/Zufall; FormMirror-Akkordeon behält geöffnete Sektionen. **v2.0.570:** geöffnete Sektionen zeigen Inhalt (kein `expand-height`-Stuck bei Höhe 0). **v2.0.572:** Accordion-Klicks setzen den Open-State zuverlässig (kein stale-Show). Stage/Showcase und `ui-bottom-collapsed` blenden das Dock weiter aus. Splitter-Greifer sitzt im 8px-Trigger auf der jeweiligen Trennlinie (nicht Bildschirmmitte). Farbkategorien (Tab Farben): Optik `ToggleGroup` outline/sm, weiterhin **eine** Kategorie. App-Start: Park `Progress` indeterminate in `#app-loading-root`.
...
- **v2.0.399:** Gesims/Sockel ohne eigene Farbe = Steine nur bei sichtbaren Paneelen, sonst Putzfarbe.
- **v2.0.398:** Gesims/Sockel/Zierband ohne eigene Farbe = sichtbare Steinfarbe (`claddingColor`, sonst `wallColor`). Nicht `profileColor`/`wallColor` allein — sonst Weiß auf beige Steinen nach Größenänderung.
- **v2.0.396:** Höhe von Sockel/Gesims bei aktiver Teil-Wahl: Facade-Shade muss auf `originalMaterial` (nicht nur auf dem orangen Overlay). Sonst bleibt die Fläche nach Abwahl grau. EnvMap allein reicht nicht (Albedo blieb `#ffffff`). Nicht in `applySelection` erneut die ganze Szene beschatten — das blockiert den Start. Himmel-CDN max. 5 s. (**v2.0.420:** Ladebildschirm wieder bis Bootstrap; Failsafe 20 s — frühes Dismiss nach State-Load war zu aggressiv.)
- **v2.0.397:** `originalMaterial` kann ein Material-**Array** sein (Laibung). Nicht das Array als Ganzes in die Shade-Liste schieben — sonst `userData` undefined, Constructor wirft, Haus bleibt unsichtbar. Arrays elementweise beschatten.
...
| Gitter / franz. Balkon | `guard.color` / `#guard-color-swatches-hub` | `guard.finish` / `#opening-guard-finish` |
...
| Dach / Decke | bei Dachwahl: **Dachfenster**, **Gauben** (10 Formen, v2.0.479), Farben | |

- **Gaube gewählt (v2.0.479 / Rechtsklick v2.0.481 / Drag v2.0.482 / Fenster-UI v2.0.483):** rechte Leiste zeigt **Maße** (Breite, Front, Tiefe, Überstand, Wandstärke) → **Form** → **Position** → **Fenster an/aus**. Bei Fenster an zusätzlich die volle **Fenster-Toolbar** (wie Fassadenfenster). Verschieben: **8 cm**, Hilfslinien bis Boden und Dachende, Pfeiltasten. Rechtsklick: Ein-/Ausblenden, Duplizieren, Traufdurchbruch, Fenster, Löschen. Details: [roof.md](roof.md).
...
- Nach Dock/Move/Rotate: `finalizeStudioGeometry` → `recomputeStudioWallMiters` aus **Nachbar-Endpunkten** (`miterAtWallEnd`), nicht aus dem Grundriss-Walk (der die Wand entgegen `yawDeg` durchlaufen kann). **45°- und 90°-Ecken:** Paneele, Sockel und Gesims als Bilderrahmen an der Front, **nur wenn dieselbe Schicht um die Ecke weiterläuft** (v2.0.252) — sonst stumpf an der Wandkante und Profile geschlossen. v0.7.220: vorzeichenbehaftetes `z × tan(Knick/2)` — auch stumpfe 45°-Außenecken.
...
| Nischen | **Fallrohr DN 80** (Gebäude-Fixture über alle Etagen) plus Cutout-Presets (eckig/rund, Durchbruch) und **Konche** (Kalotte, Presets 96×128 / 64×96) | Fallrohr/Cutouts auf Wand droppen; Fallrohr: Drag/Pfeile, optional Schmuck-Durchbruch + Nische; **v2.0.409:** Aufsatz **Wandabstand** (cm) einstellbar — siehe [downpipes.md](downpipes.md); Cutouts ohne Glas; **v2.0.405:** bei Cutout/Konche auch Bibliothek-Tab **Profile** (Rahmen wie Fenster; Fallrohr-Nischen ausgenommen); Konche: Rundbogen-Loch + Halbzylinder/Viertelkugel |
...
- **Wetter / Schneefall** (v2.0.553) — unter Szene → Wetter; Temp, Intensität, Qualität. Details: [snow.md](snow.md).
...
- **Szene abspielen (v2.0.254 / v2.0.255):** einmalige Animation mit **Dauer (5…120 s)** und Modus:
  ...
  - **v2.0.255:** Playback im Hauptloop, Scrub-Schattenpfad (kein Debounce-Ruckeln), gedrosselte Toolbar-Updates, Smoothstep.

Voraussetzungen für laufende Sonne (Zyklus): **Tageszyklus an** + **Animationen pausieren aus** + nicht im Licht-Modus + kein einmaliges Abspielen.
...
- **Abstände (UI-Kit / v2.0.385 / v2.0.395):** innerhalb einer Sektion immer **16 px** (`--space-3`); **zwischen Sektionen 32 px** (`--space-4`). Inhalt **16 px vom Leistenrand**; Titel links, Control rechts — keine Stepper/Selects über den Rand. Label→Feld 4 px; Aktions-Buttons Gap 8 px. Kein Gap+Margin-Doppel. Details: [ui-kit.md](ui-kit.md).
