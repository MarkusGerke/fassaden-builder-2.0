...
Im Reiter **Schrift**: Textfeld, darunter **Schriftart**-Karten wie die Paneel-Vorschau. Jede Karte ist **16:9**, die Karten stehen **untereinander**. Die Vorschau zeigt denselben Text wie das Feld oben, gesetzt in der jeweiligen Schrift (leer → Platzhalter des Textfelds). Standard bleibt **Federo**. **v2.0.265:** Der rechte Reiter erscheint erst, wenn die Wand schon Schrift hat — Anlegen über die **Bibliothek**.

**Bibliothek (unten, Tab Schrift) — v2.0.241 / v2.0.267:** Wie Wandöffnungen. Ohne Schrift-Fokus: **Klick** legt eine weitere Instanz an; **Drag&Drop** platziert am Drop-Punkt. **Mit Fokus** auf eine Schrift (`selectedWallPart === 'label'`): aktive Schriftart ist in der Bibliothek markiert (`library-card-applied`); Klick **tauscht** `fontId` der Auswahl (`selectLabelFont`), legt keine neue Instanz an. Während des Ziehens orangener Platzhalter. MIME `application/x-label-font`. Raster **8 cm**.

Auswahl in 3D (**v2.0.267**): nur die gewählte Instanz (`selectedLabelId`) bekommt eine orangene Kontur; der Text bleibt sichtbar (kein solides Auswahl-Material auf dem Glyph).
...
`wall.labels[].fontId` / Legacy `wall.label.fontId` → `resolveLabelFontId` → TTF (`FontFace`/Canvas) bzw. `*.typeface.json` (`TextGeometry`). Bibliothek: neue Instanz über `placeLabelFromLibrary` / `addWallLabel`; bei bestehender Auswahl Schriftart tauschen über `selectLabelFont` / `commitLabelPatch({ fontId })`.
