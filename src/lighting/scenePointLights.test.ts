...
describe('scenePointLights', () => {
  it('erzeugt Innen- und Außen-Punktlicht mit sichtbaren Markern', () => {
    ...
    expect(bundle.indoorMarker).toBeInstanceOf(THREE.Sprite)
  })
  ...
  it('skaliert Intensität bei Nacht hoch', () => {
    ...
    expect(bundle.indoor.intensity).toBeCloseTo(wattsToThreeIntensity(8), 0)
    expect(bundle.outdoor.intensity).toBeCloseTo(wattsToThreeIntensity(12), 0)
  ...
  it('dimmt bei Tag, Marker bleiben sichtbar', () => {
    ...
    expect(bundle.indoor.intensity).toBeGreaterThan(wattsToThreeIntensity(8 * 0.35))
    expect(bundle.indoorMarker.visible).toBe(true)
  })

  it('blendet Marker aus wenn showMarkers false', () => {
    const bundle = createScenePointLights()
    const box = new THREE.Box3(new THREE.Vector3(0, 0, 0), new THREE.Vector3(400, 448, 300))
    updateScenePointLights(
      bundle,
      box,
      { ...DEFAULT_SUN_SETTINGS, timeOfDay: 2 },
      { enabled: true, castShadow: true, showMarkers: false },
    )
    expect(bundle.indoorMarker.visible).toBe(false)
    expect(bundle.outdoorMarker.visible).toBe(false)
  })
})
